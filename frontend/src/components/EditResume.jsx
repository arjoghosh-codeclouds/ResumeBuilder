...
const useResizeObserver = () => {    //resizeobserver api
 ...
 const [resumeData, setResumeData] = useState({
  ...
  const validateAndNext = () => {
    ...
    switch (currentPage) {
      ...
      case "additionalInfo":
        if (resumeData.languages.length === 0 || !resumeData.languages[0].names?.trim()) {
...
const fetchResumeDetailsById = async () => {
  try {
    ...
    if (response.data && response.data.profileInfo) {
      ...
      setResumeData({
        ...resumeInfo, // directly set everything from backend
        template: {
          theme: resumeInfo?.template?.theme || "modern",
          colorPalette: resumeInfo?.template?.colorPalette || [],
        },
      });
    }
  } catch (error) {
  ...
  const uploadResumeImages = async () => { 
    try {
      ...
      const fixedThumbnail = fixTailwindColors(thumbnailElement)
       //first it is fixing the tailwind colours
      const thumbnailCanvas = await html2canvas(fixedThumbnail, {
        scale: 0.5,
        backgroundColor: "#FFFFFF",
        logging: false,
      })
      //then it is converting to html canvas using html2canvas
  ...
  const updateResumeDetails = async (thumbnailLink) => {
      try {
        setIsLoading(true)
        console.log("Resume ID to Update:", resumeId);
...
const downloadPDF = async () => {
    ...
    override.textContent = `
      * {
      ...
      .p-6.bg-white.font-sans.text-gray-800 {
    ...
    try {
      await html2pdf()
        .set({
          // Set standard print margins
          margin: [10, 10, 10, 10], 
          filename: `${resumeData.title.replace(/[^a-z0-9]/gi, "_") || "resume"}.pdf`,
          ...
          html2canvas: {
            ...
            backgroundColor: "#FFFFFF",
            logging: false,
            // Use a fixed width to stabilize the layout (adjust 800px if needed)
            windowWidth: 800, 
            scrollY: 0,
            scrollX: 0
          },
          ...
          pagebreak: {
            // Use 'css' mode for better control over page splits
            mode: ['css', 'legacy']
          }
        })
        .from(element)
      ...
      // Removed err.message to avoid showing the technical 'oklch' error to the user
      toast.error("Failed to generate PDF. The template may contain unsupported styles.");
